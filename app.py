...
from exceptions import APIException, CryptoConverter, DeclensionByCases
...
def help(message: telebot.types.Message):
    text = 'Чтобы перевести валюту, напишите мне команду следующим образом:' \
           ...
           '\nНапример: рубль евро 1\n' \
           '\nЧтобы увидеть валюты, которые я могу конвертировать, введите команду\n/values'
    ...
    try:
        ...
        if len(values) != 3:
            raise APIException('\nКоличество параметров не совпадает!\n'
                                      ...
                                      '\nИначе между нами возникнет недопонимание :(')

        quote, base, amount = values
        total_base = CryptoConverter.get_price(quote, base, amount)
    except APIException as e:
        bot.reply_to(message, f'Ой Вы что-то не так написали:\n{e}')
    except Exception as e:
        bot.reply_to(message, f'Что-то пошло не так :(\n\n{e}\n'
                              f'\nНо это не Ваша вина, просто сегодня такой день.')
    else:
